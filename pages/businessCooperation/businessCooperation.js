...
Page({
  ...
  data: {
    ...
    condition: false,
    // 地区选择相关
    shopAddress: 'Add map markers',
    showMain: true,
    allHidden: false,
    hiddenMain: false,
    // 图片上传
    addPicText: 'Upload Now',
    ...
    upStatus: 0,
    license: [{ name: 'noraml', value: 'Business License', checked: 'true' }, { name: 'special', value: 'Franchise Documents' }],
    licenseTime: [{ name: 'noTime', value: 'Long-term Effective', checked: 'true' }, { name: 'haveTime', value: 'Fixed Term' }],
    restaurantLicenseTime: [{ name: 'noTime', value: 'Long-term Effective', checked: 'true' }, { name: 'haveTime', value: '固定有效期' }]
  },
  /**
   * 信息录入
   * @param e
  ...
  nextStep2: function nextStep2() {
    if (!this.data.xkzAddress || !this.data.xkzNumber || !this.data.xkzName || !this.data.zzAddress || !this.data.zzNumber || !this.data.zzName || !this.data.frIdNumber || !this.data.frName || this.data.IdFaceImg.indexOf('wxfile') === -1 || this.data.licenseImg.indexOf('wxfile') === -1 || this.data.restaurantLicenseImg.indexOf('wxfile') === -1) {
      return wx.showModal({
        title: 'Sorry',
        content: 'Please complete your qualification information before going to the next step',
        showCancel: false
      });
    }
    this.setData({
  ...
  radioChange: function radioChange(e) {
    console.log('radio activate change event, the value is：', e.detail.value);
  },
  ...
  addMapSite: function addMapSite() {
    ...
    wx.chooseLocation({
      success: function success(res) {
        // console.log(res)
        that.setData({
          shopAddress: res.name || res.address || 'Add map markers'
        });
      }
    });
  ...
  nextStep: function nextStep() {
    if (this.data.shopAddress === 'Add map markers' || this.data.insideImg.indexOf('wxfile') === -1 || this.data.faceImg.indexOf('wxfile') === -1 || !this.data.addressDetail || !this.data.lxrName || !this.data.lxrPhone) {
      return wx.showModal({
        title: 'Sorry',
        content: 'Please complete your qualification information before going to the next step',
        showCancel: false
